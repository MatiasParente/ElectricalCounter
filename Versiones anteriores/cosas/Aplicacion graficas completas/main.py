...
#Menu
    ...
    def Select_Datos_Consumo_Dia(self):
        ...
        if connection is not None:
            try:
                cursor = connection.cursor()
                query = "SELECT Dato.Kwh, Plan_UTE.PrecioP FROM Dato JOIN Usuario ON Dato.UsuarioDispo = Usuario.CodigoUsuario JOIN Plan_UTE ON Usuario.PlanUteID = Plan_UTE.codigoP WHERE Usuario.NombreUsuario = %s AND DATE(Dato.Fecha) = %s AND TIME(Dato.Fecha) >= '00:00:00' AND TIME(Dato.Fecha) <= '23:59:59';"
    ...
    def Select_Datos_Consumo_Mes(self):
        ...
        if connection is not None:
            try:
                ...
                ultimo_dia_mes_actual = datetime.now().replace(day=1, month=datetime.now().month % 12 + 1, year=datetime.now().year if datetime.now().month < 12 else datetime.now().year + 1) - timedelta(days=1)
                ultimo_dia_mes_actual = ultimo_dia_mes_actual.strftime('%Y-%m-%d')
                query = "SELECT Dato.Kwh, Plan_UTE.PrecioP FROM Dato JOIN Usuario ON Dato.UsuarioDispo = Usuario.CodigoUsuario JOIN Plan_UTE ON Usuario.PlanUteID = Plan_UTE.codigoP WHERE Usuario.NombreUsuario = %s AND DATE(Dato.Fecha) >= %s AND DATE(Dato.Fecha) <= %s;"
...
# Pagina Controles
    ...
    def publish_mqtt_message(self, message):
        self.client.connect("192.168.144.52", 1883, 1)
