...
#Login Registrar
    ...
    def register(self):
        ...
        if plan_ute is None:
            self.show_message("Error", "Registro no válido.")
            return
        ...
        try:
            ...
            self.show_message("Error", "Datos ingresados no válidos.")
...
#Pagina Usuario
    ...
    def actualizar_variable_Usuario(self,  NombreU, CorreoU, DispoU, NombrePU):
        if NombreU and CorreoU and DispoU and NombrePU:
            ...
            self.root.get_screen('screen3').ids.CorreoE.text = f"      Correo electrónico : {CorreoU[-1]}"
            self.root.get_screen('screen3').ids.NuemeroD.text = f"      Numero de dispositivo :{DispoU[-1]}"
